...
for step1 in num[3]
    for a in n1[3]
        ...
        global data2 = [h_data v_data u_data]
        ...
        opt = ADAM(η);
        data = Iterators.repeated((x11,y11),5000)
        ...
        Flux.train!(loss,Flux.params(model),data,ADAM(0.01),cb = cb!)
        data = Iterators.repeated((x11,y11),5000)
        Flux.train!(loss,Flux.params(model),data,ADAM(0.005),cb = cb!)
        data = Iterators.repeated((x11,y11),5000)
        Flux.train!(loss,Flux.params(model),data,ADAM(0.001),cb = cb!)
...
baby_sat_model = model
@save "baby_sat_model.bson" baby_sat_model
...
""";
@load "baby_sat_model.bson" baby_sat_model
er_train1 = reshape(err_train,iter,length(x1)) # This is train error for each model
er_test1 = reshape(err_test,iter,length(x1)) # test error for each model
...
##
"""PLOTTING TRAIN VS TEST ERROR FOR EACH MODEL""";

"""
We observe that all the train and test error of same order i.e low variance
and low bias, its a good model
"""
P2 = plot(;title = "Train vs test error η = $(η), with $(n)
Hidden units Iterations = $(iter)")
for i in 2 #:length(x1) change from 1 to 9
    P2 = plot!(er_train1[:,i],xlabel = :"Iterations",ylabel = "log10(error)",yaxis = :log10,
    label = "$(idx[i][1]) steps, $(idx[i][2])% variation ",legend= :best)
    P2 = plot!(er_test1[:,i],xlabel = :"Iterations",ylabel = "log10(error)",yaxis = :log10,
    label = "$(idx[i][1]) steps, $(idx[i][2])% variation ",legend= :best)
end
display(P2)
...
# predvec predicts only which model is best trained for its own data.
...
using BSON: @load
using Flux
@load "model_vec1000 0.005 16.bson" model_vec
#plot(plot((vec(model_vec[i](xt[i]))-vec(yt[i]) for i in 1:9)...))
pv1=[];
for i in 9 #:length(x1)
        for j in 1 :length(x1)
                 push!(pv1,model_vec[i](xt[j]))
        end
        l1error = [abs.(vec(pv1[i])-vec(yt[i])) for i in 1:9]
        display(plot([scatter(l1error[i],title = "on $(maximum(size(xt[i]))) data") for i in 1:9]...,))
end


""" Error of each model for the predicted initial conditions """;
err_vec = [pred_vec[:,i]-utd for i in 1:9]
for i in 1:9
    display(plot(scatter(( err_vec[i])))) #,layout = (3,3))))
end
