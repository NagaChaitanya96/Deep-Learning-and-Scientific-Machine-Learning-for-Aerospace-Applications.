""" U ~ sigmoid with random initial parameters, with OBJECTIVE FUNCTION as TRAPIZOIDAL RULE + penality starting from 1e-2 """;
"""
a) parameters obtained are local values as per the initial guess chosen, as part of minimizing the objective function its observed
that "c" is obtained close to 3.0 without giving any additional boundary condition for Thrust.

b) Penalty taken > 0.1 will lead the Npde to minimize only the addloss, than the PDE,BC so conditions are not satisfied.
...
function ThrustLossfunc(phi,θ,p)
      ...
      J = abs((δt/2) * sumofall)* 1e-3
      #println("params are",p)
      #println("loss Thrust ",J)
      return J
end
...
begin
      parameters = [a,b,c]
      pguess2 = [30,50,16]
      initGuess = Dict([p => pguess2[i] for (p,i) in zip(parameters,[1,2,3])])
...
png("D:\\Chaitanya\\sciml\\Analysis so far\\Final2 - Chosing better penalty")
png("D:\\Chaitanya\\sciml\\Analysis so far\\Final2.1 - Chosing better penalty")
