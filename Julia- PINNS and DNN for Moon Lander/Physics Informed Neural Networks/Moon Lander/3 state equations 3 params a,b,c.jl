...
MOON LANDER with taking U as a neural network and assuming the constraints to be
                  U(0) ~ 0. and u(tf) ~ 3. + No additional loss
...
begin
      ...
      """ parameters obtained for getting analytical udata are  a = 157, b = 221, c = 3  """
      eqn = [Dt(h(t)) ~ v(t),
                  Dt(v(t)) ~ -1.5 + c*1/(1+exp(-a*t+b)) ,
                  Dt(u(t)) ~ c*a*(exp(-a*t+b)/(1+exp(-a*t+b))^2)]
...
begin
      parameters = [a,b,c]
      initGuess = Dict([p => 0.01 for p in parameters])
...
png("D:\\Chaitanya\\sciml\\Analysis so far\\ 3params bad initial guess leads to wrong results")



"""
Conclusion:
      1. For C = 3 and initial guess of a and b we will be able to get the approximate curve.
      2. If assuming C also as a parameter which will be multiplied with sigmoid function, we need a good initial guess
         to start with else, we end up with inaccurate approximation.

         i.e for different iterations we are getting different values of a,b,c; if we chose a wrong initial guess ,
         since dont have any constraint
         for our objective function included.
"""
