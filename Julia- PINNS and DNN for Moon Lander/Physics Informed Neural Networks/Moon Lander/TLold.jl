...
#=
eqn = [Dt(h(t)) ~ v(t),
            Dt(v(t)) ~ -1.5 + 1/(1+exp(-(a+b*t))) ,
            Dt(u(t)) ~ b*(exp(a+b*t)/(1+exp(a+b*t))^2)]
=#
begin
      v0= -2.; h0 = 10.
...
begin
      ...
      function TL(phi,θ)
            ...
            penalty = 1
            J1 = (δt/2)*sumofall*penalty
            return J1
      end
      t2 = reduce(vcat,t_)
      function Hpenality(phi,θ)
            uₚ(i) = ([first(phi[i](t,θ[sep[i]])) for t in t_]) #θ = res.minimizer
            up1 =  reduce(vcat,uₚ(1)) #([first(phi[1](t,θ[sep[1]])) for t in t_]) # θ = res.minimizer
            u21 =  [(up1[i] >=0.0) ? up1[i] : up1[i] + exp(-10*up1[i].*t2[i]) for i in 1:length(t_)]
            J2 = sum(u21)
            return J2
      end
      ...
      addloss(phi,θ,p) = Hpenality(phi,θ)+TL(phi,θ)
      #discretization = PhysicsInformedNN(chain,grid_strategy,param_estim = true ,additional_loss = nothing)
      #discretization = PhysicsInformedNN(chain,grid_strategy,param_estim = true ,additional_loss = Hpenality)
...
begin
      ...
      p12 = plot(ts, u_predict, label = ["h(t)" "v(t)" "u(t)"],yticks = -4.:1.:10,xticks = 0.0 :0.5: 4.2, title = "w/o data $(points)pnts O(n^5)")
      plot(p11,p12)
end
